        ...
        >
          <div class="card-body">
            <ul v-if="semestre.ues.length > 0" class="list-group">
              ...
              >
                <div class="accordion" v-bind:id="`accordionFlush${ue.id}`">
                  <div class="accordion-item">
                    <h2 class="accordion-header" v-bind:id="`flush-h${ue.id}`">
                      <button v-on:click="openModel(ue.id)" class="accordion-button collapsed" type="button" data-bs-toggle="collapse" v-bind:data-bs-target="`#flush-${ue.id}`" aria-expanded="false" v-bind:aria-controls="`flush-${ue.id}`">
                        {{ ue.designation }}
                      </button>
                    </h2>
                    <div v-bind:id="`flush-${ue.id}`" class="accordion-collapse collapse" v-bind:aria-labelledby="`flush-h${ue.id}`" v-bind:data-bs-parent="`#accordionFlush${ue.id}`">
                    ...
                    <tr v-for="(note, i) in enote" :key="note.id">
                        <template v-if="i == 0">
                            <td v-bind:rowspan="enote.length">
                              {{ ecue }} 
                              <router-link :to="`/ecue/epv/form/${enote[i].ecue.id}`" class="btn btn-primary btn-sm">
                                  E
                              </router-link>
                              </td>
  ...
  methods: {
    ...
    sendMetaData() {
      this.$emit("getMetaData", {
        title: "Semester List",
        description: "List of Semesters with UES",
      });
    },
  },
  mounted() {
